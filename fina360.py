...
async def main():
    ...
    # reduce capture resolution to lower CPU/GPU load when possible
    try:
    ...
    PROCESS_EVERY_N_FRAMES = 10  # only run expensive transform every N frames
    ...
    # Pre-load face detection DNN once to avoid repeated loads (can be slow first time)
    try:
    ...
    while True:
        ...
        if frame_count % PROCESS_EVERY_N_FRAMES == 0:
            ...
            if not all_faces:
                cv2.imshow("OUTPUT", np.zeros((640, 640, 3), dtype=np.uint8))
                continue
        ...
        for _name in ('frame', 'frame0', 'gray', 'size','centers', 'radius', 'all_faces', 'best', 'best_bbox', 'face_center_x', 'chosen_center', 'faces', 'shape', 'image_points', 'camera_matrix', 'dist_coeffs', 'focal_length', 'center', 'rotation_vector', 'translation_vector', 'rmat', 'proj_matrix', 'euler_angles', 'yaw', 'pitch', 'roll', 'axis', 'imgpts'):
            if _name in locals():
                try:
                    del locals()[_name]
                except Exception:
