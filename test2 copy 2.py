...
async def main():
    ...
    print("Use on-screen buttons or keyboard 'q' to quit.")
    print("Default: Frontalize OFF")
    mode = 0  # 0: OFF, 1: ON
    ...
    frame_count = 0
    # set up simple UI state and mouse callback for buttons
    ui_state = {'mode': mode, 'quit': False, 'buttons': []}
    ...
    def draw_buttons(frame, state):
        ...
        x = margin
        y = margin
        labels = [("Frontalize ON", 1), ("Frontalize OFF", 0), ("Quit", 'quit')]
        rects = []
        for label, action in labels:
            x1, y1 = x, y
            x2, y2 = x + btn_w, y + btn_h
            # background
            cv2.rectangle(frame, (x1, y1), (x2, y2), (30, 30, 30), -1)
            # outline highlight for selected mode
            if isinstance(action, int) and action == state.get('mode'):
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 200, 0), 2)
            else:
                cv2.rectangle(frame, (x1, y1), (x2, y2), (80, 80, 80), 1)
            # text
            color = (200, 200, 200) if action != 'quit' else (220, 180, 180)
            cv2.putText(frame, label, (x1 + 10, y1 + 26), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2, cv2.LINE_AA)
            rects.append((x1, y1, x2, y2, action))
            y += btn_h + spacing
        state['buttons'] = rects
    ...
    def on_mouse(event, mx, my, flags, param):
        if event != cv2.EVENT_LBUTTONDOWN:
            return
        for (x1, y1, x2, y2, action) in ui_state.get('buttons', []):
            if x1 <= mx <= x2 and y1 <= my <= y2:
                if action == 'quit':
                    ...
                    print(f'Frontalize set to: {"ON" if action==1 else "OFF"}')

    cv2.namedWindow("OUTPUT", cv2.WINDOW_NORMAL)
    # force output display size to SEND_MAX_WIDTH x SEND_MAX_WIDTH (default 512)
    try:
        cv2.resizeWindow("OUTPUT", SEND_MAX_WIDTH, SEND_MAX_WIDTH)
    except Exception:
        # if resizeWindow fails (older OpenCV), silently continue
        pass
    # control window for separate buttons
    cv2.namedWindow("CONTROL", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("CONTROL", 200, 160)
    cv2.setMouseCallback("CONTROL", on_mouse)

    def make_control_image(state, width=200, height=160):
        img = np.zeros((height, width, 3), dtype=np.uint8) + 40
        draw_buttons(img, state)
        return img
    while True:
        # update mode/quit from UI state (mouse callbacks)
        if ui_state.get('quit'):
            break
        mode = ui_state.get('mode', mode)
        
        # update control window every frame for responsive UI
        ctrl = make_control_image(ui_state)
        cv2.imshow("CONTROL", ctrl)
        ...
        if frame_count % PROCESS_EVERY_N_FRAMES == 0:
            ...
            if not all_faces:
                empty = np.zeros((SEND_MAX_WIDTH, SEND_MAX_WIDTH, 3), dtype=np.uint8)
                cv2.imshow("OUTPUT", empty)
                # allow keyboard quit as backup
